...
def runexp():
    ...
    #    
        ...
        for trial in all_trials:       
            ...
            clue  = prep_cont(trial['clue'],fixa_pos,text_h)
            task = trial['task']
            Rating1 = prep_cont(trial['Question1'],clue_pos,text_h)
            Rating2 = prep_cont(trial['Question2'],clue_pos,text_h)
            Rating3 = prep_cont(trial['Question3'],clue_pos,text_h)
            ...
            border = visual.Rect(win, width = 500, height = 120, fillColor = 'white', lineColor = 'black',pos = (0,0))
            border.draw()
            clue.draw()
            ...
            task = visual.TextStim(win,text = upper_task,color = win_text_col,pos = clue_pos,height = text_h)
            task.draw()
            ...
            if trial['trial type'] == 'catch trial':
                ...
                for word_index in range(15):
                    ...
                    if word_index == random_i:
                        ...
                        word = visual.TextStim(win,text = word,color = changed_col,pos = clue_pos,height = text_h)
                    else:
                        word = visual.TextStim(win,text = word,color = win_text_col,pos = clue_pos,height = text_h)
                ...
                for word_index in range(15):
                    ...
                    word = visual.TextStim(win,text = word_text,color = win_text_col,pos = clue_pos,height = text_h)
                    word.draw()
                    timetodraw = fixa1_onset + fixa_list[fixa_num] + clue_time + task_instr_time + word_time * word_index
                    while core.monotonicClock.getTime() < (timetodraw - (1/120.0)):
                        pass
            ...
            if trial['task'] != 'xxxxxxxx':
                ...
                ratingScale = visual.RatingScale(win, maxTime = 4, low = 1, high = 7, markerStart=4, tickMarks = [1,2,3,4,5,6,7], textColor='Black', leftKeys='1', rightKeys = '2', acceptPreText='Please select your rating', acceptKeys= '4', acceptText='accept?', acceptSize = 2.0, marker='slider', stretch=2, tickHeight=1.5,scale = None)
