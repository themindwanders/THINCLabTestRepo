...
def runexp(backup_log, timer, win, writer, resdict, numtrial):
    ...
    prompts = []
    with open("resources/Memory_Task/memory_prompts.csv", "r") as f:
    ...
    for enum, thisTrial in enumerate(trials):
        currentLoop = trials
        # abbreviate parameter names if possible (e.g. rgb = thisTrial.rgb)
        if thisTrial != None:
            for paramName in thisTrial:
        ...
        resdict['Timepoint'] = (prompts[enum][0].strip().strip(), "started")
        ...
        for thisComponent in trialComponents:
            ...
            thisComponent.text = prompts[enum][0].strip().strip()
        ...
        resdict['Timepoint'] = (prompts[enum][0].strip().strip(), "ended")
        resdict['Time'] = timer.getTime()
        writer.writerow(resdict) 
...
#runexp(backup_log, timer, win, writer, resdict, numtrial)